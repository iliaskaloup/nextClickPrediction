# -*- coding: utf-8 -*-
"""
Created on Tue Sep 17 21:11:12 2019
...
from keras.layers import TimeDistributed, CuDNNGRU
...
training_data=pd.read_csv('RecSysFullData.csv', header=None, sep =',')
#training_data=training_data.iloc[0:467150,:]
print("hello2")

#euresh new session_lenght
lenght=pd.read_csv('lenghtRecsys100.csv', header=None, sep =',')
lenght=lenght.iloc[:,0]
...
lenght=pd.DataFrame(lenght)
lenght.to_csv ('lenghtRecsys100.csv', index = None, header=False)
'''
...
model=Sequential()
model.add(Masking(mask_value=0, input_shape=(timesteps-1, len(itemId2))))
model.add(GRU(units=200, kernel_initializer=my_init, return_sequences=True, stateful=False))  
#model.add(GRU(units=200, kernel_initializer=my_init, return_sequences=True))  
...
model.add(Activation('tanh'))
'''
model.add(TimeDistributed(Dense(units = len(itemId2), kernel_initializer=my_init)))
#model.add(Dense(units = len(itemId2), kernel_initializer=my_init))
model.add(Activation('softmax'))#Output
#model.add(Activation('tanh'))#Output, paei mazi me binary ce
#activation function 
#sgd = optimizers.Adagrad(lr=0.01, decay=1e-6)
...
sgd = optimizers.Nadam(lr=0.002, beta_1=0.9, beta_2=0.999, schedule_decay=0.004)
...
#mc = ModelCheckpoint('best_model.h5', monitor='val_rec', mode='max', verbose=1, save_best_only=True)
history = model.fit_generator(generator(dataFrameTrain,BS,train_lenght,itemId2),validation_data=next(iterator2),epochs=nb_epoch, callbacks=[csv_logger,es],steps_per_epoch = int(trainSize//BS),validation_steps=int(testSize // BS),use_multiprocessing=False)
#history = model.fit_generator(generator(dataFrameTrain,BS,train_lenght,itemId2),epochs=nb_epoch, callbacks=[csv_logger],steps_per_epoch = int(trainSize//BS),use_multiprocessing=False)
...
path='modelRecsysFull3x256.h5'
model.save(path)
#model = load_model('modelRecsysFull.h5')
from contextlib import redirect_stdout
with open('modelsummaryRecSysFull.txt', 'w') as f:
...
print("training time %d"%(milli_sec2-milli_sec1))
